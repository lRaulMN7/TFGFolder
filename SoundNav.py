...
class SoundNavLogic(ScriptedLoadableModuleLogic):
  ...
  def createParameterNode(self):
    ...
    parameterNode.SetParameter("MaxNumberOfInstruments", "5")
  ...
  def instrumentNodeUpdated(self, instrumentIndex):
    ...
    slicer.vtkMRMLTransformNode.GetMatrixTransformBetweenNodes(
        NeedleNode,
        parameterNode.GetNodeReference("InstrumentReference0"),
        NeedleToReferenceMatrix)

    ProbeToReferenceMatrix = vtk.vtkMatrix4x4() 
    slicer.vtkMRMLTransformNode.GetMatrixTransformBetweenNodes(
        ProbeNode,
        parameterNode.GetNodeReference("InstrumentReference1"),
        ProbeToReferenceMatrix)

    PlaneToReferenceMatrix = vtk.vtkMatrix4x4() 
    slicer.vtkMRMLTransformNode.GetMatrixTransformBetweenNodes(
        PlaneNode,
        parameterNode.GetNodeReference("InstrumentReference2"),
    ...
    grados = (math.asin(iCom*Nox + jCom*Noy + kCom*Noz/(math.sqrt(pow(iCom, 2)+pow(jCom, 2)+pow(kCom, 2))*math.sqrt(pow(Nox, 2)+pow(Noy, 2)+pow(Noz, 2))))*180)/(4*math.atan(1))
